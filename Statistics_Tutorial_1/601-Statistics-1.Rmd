...
The p-value here is `r chisq.test(freq_table_mtcars)$p.value`, indicating a relatively high probability that we would have observed these data if the null hypothesis were true. We would fail to reject the null hypothesis here, retaining our belief that engine shape and transmission type are unrelated.
...
For example, some researchers may collect more data until their p-value falls below the typical rejection criterion of .05. This practice is known as **p-hacking** and can result in the publication of erroneous results. Others have criticized the .05 criterion as being too liberal and suggest that researchers adopt a more conservative threshold (such as .001). One final critique of NHST in practice comes from researchers misinterpreting the definition of a p-value. A p-value does **NOT** give you the probability the null hypothesis is true. Rather, it gives you the probability of observing your data given that the null is true (i.e., $P(Data|Null)$).
