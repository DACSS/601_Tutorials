...
```
 ...
 Standardize the vector by subtracting the mean of `vector_1` from each score and then dividing by `vector_1`'s standard deviation. Then, use `microbenchmark()` to measure the computation time in \textbf{seconds}, and plot your results.
